...
class IndexAction extends BaseAction {
    ...
    public function index(){
        ...
        $shou = [];
        $sql_cs = "select a.id,a.product_title,a.volume,a.imgurl,c.price ,a.leve1,a.leve2
...
where a.status = 0 and a.num >0 and s_type like '%4%' 
 order by a.volume desc limit  0,10";
        ...
        foreach ($r_c as $key => $value) {
            ...
            $sql_s = "select a.id,a.product_title,a.volume,c.price,a.imgurl ,a.leve1,a.leve2
...
//                $leve1 =$r_ttt[0]->leve1;
//                $leve2 =$r_ttt[0]->leve2;
//                ,'leve1' => $v->leve1,'leve2' => $v->leve2
                $product[$k] = array('id' => $v->id,'name' => $v->product_title,'price' => $price,'price_yh' => $price_yh,'imgurl' => $imgurl,'volume' => $v->volume,'leve1' => $v->leve1,'leve2' => $v->leve2);
            }
    ...
    public function get_more(){
        ...
        if($index == 0){
            ...
            $sql_cs = "select a.id,a.product_title,a.volume,a.imgurl,a.leve1,a.leve2
...
group by c.pid  order by a.volume desc limit  $start,$end";
            ...
            if($r_cs){
                foreach ($r_cs as $keyc => $valuec) {
                    ...
                    $valuec->imgurl = $img . $valuec->imgurl;
                    $product[] = array('id' => $valuec->id,'product_title' => $valuec->product_title,'price' =>$valuec->yprice,'price_yh' => $valuec->price,'imgurl' => $valuec->imgurl,'volume' => $valuec->volume,'leve1' => $valuec->leve1,'leve2' => $valuec->leve2);
                }
            }
         ...
         echo json_encode(array('prolist'=>$product,'status'=>1));
            ...
            //查询商品并分类显示返回JSON至小程序
            $sql_t = "select a.id,a.product_title,a.volume,c.price,a.imgurl ,a.leve1,a.leve2
from lkt_product_list AS a 
RIGHT JOIN (select min(price) price,pid from lkt_configure group by pid)  AS c 
ON a.id = c.pid 
where a.product_class like '%-$ttcid-%' and a.status = 0 and a.num >0
order by a.sort DESC LIMIT $start,$end";
            ...
            foreach ($r_s as $k => $v) {
                ...
                $price =$r_ttt[0]->yprice;
                $price_yh =$r_ttt[0]->price;
                $product[$k] = array('id' => $v->id,'product_title' => $v->product_title,'price' => $price,'price_yh' => $price_yh,'imgurl' => $imgurl,'volume' => $v->volume,'leve1' => $v->leve1,'leve2' => $v->leve2);
            }
                    echo json_encode(array('prolist'=>$product,'status'=>1));
                    exit;
            }else{
